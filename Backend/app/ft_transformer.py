...
class FourierTransformer:
    ...
    def create_rect_region_mask(
        ...
        # Wrap rectangles because the unshifted FFT stores the centered view's
        # edges at opposite sides of the array.
        if x2 > x1 and y2 > y1:
            x_indices = np.arange(x, x + width) % w
            y_indices = np.arange(y, y + height) % h
    ...
    def mix_components(
        ...
        mag_accum = np.zeros(target_shape, dtype=np.float64)
        phase_accum = np.zeros(target_shape, dtype=np.float64)
        ...
        if component_mode == "magnitude_phase":
            mixed_ft = mag_accum * np.exp(1j * phase_accum)
    ...
    def _to_display(self, image: np.ndarray) -> np.ndarray:
        ...
        # Limit isolated reconstruction spikes so they do not hide the image structure.
