...
class ImageManager:
    ...
    def get_all_images(self) -> Dict:
        ...
        return {
            ...
            'common_size': (
                {
                    'width': self.common_size[0],
                    'height': self.common_size[1],
                }
    ...
    def _update_common_size(self) -> None:
        ...
        # Keep the largest loaded canvas so a small image does not downscale
        # every other input and degrade the mixing result.
        target_width = max(img.size[0] for img in loaded_images)
        target_height = max(img.size[1] for img in loaded_images)
        
        self.common_size = (target_width, target_height)